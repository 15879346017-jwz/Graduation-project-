            ...
            geolocation.getCurrentPosition(function(r){
                if (this.getStatus() == BMAP_STATUS_SUCCESS) {
                    ...
                    geoc.getLocation(point, function(rs){
                        ...
                        myFun(result,num,shops_orderby,shops_where,"shop_"+customModuleId,shops_style,is_bd);
                    });
                }
...
//根据当前城市获得数据
function myFun(result,num,sortby,filter,obj,style,bd_tag){
    var ids     = [],
    ...
    $.getJSON(linki,param,function(e) {
        ...
        var arr= ids.join(",");
        ajax_shop_list(arr,'',obj,style,bd_tag);
    });
}
...
function ajax_shop_list(arr,dis,obj,style,bd_tag){
    $.ajax({
        url:'/wap/wapAjaxModule.php',
        type:'post',
        data:{username:user_name,arr:arr,is_bd:bd_tag,distances:dis,style:style,t:UTCTimeDemo},
        success: function(data){
            $('#'+obj).html(data);
            if(style==4){
                editPicHeight(obj);
    ...
    function get_city_url(new_city_name){
        ...
        $.ajax({
            ...
            success: function(data){
                if(data){
                    if (!is_bianji){
                        if(user_name=='yangguangvip' || user_name=='wjzp2018'){
