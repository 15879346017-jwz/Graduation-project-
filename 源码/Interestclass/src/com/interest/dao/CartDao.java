...
public class CartDao {
	...
	public int addCart(Cart cart) {
		String sql = "INSERT INTO interest_cart(adminuser_id,goods_id,cart_addnum,cart_addtime,cart_price) " +
	...
	public int updateCart(Cart cart) {
		String sql = "update interest_cart set adminuser_id=:adminuser_id,goods_id=:goods_id,"
	...
	public int deleteCart(int cart_id) {
		String sql = "delete from interest_cart where cart_id=:cart_id";
		MapSqlParameterSource sps = new MapSqlParameterSource();
	...
	public List<Cart> findCartByPage(Cart cart, PageBean pageBean) {
		String sql = "select c.*,a.adminuser_username as adminuser_username,f.course_name as goods_name,f.course_images as goods_images,"
				+ "f.course_price as goods_price from interest_cart c,interest_adminuser a,interest_course f "
				+ "where c.adminuser_id=a.adminuser_id and c.goods_id=f.course_id limit " + pageBean.getStart()
	...
	public int CartCount(Cart cart) {
		String sql = "select count(*) total from interest_cart";
		int totals = 0;
		MapSqlParameterSource sps = new MapSqlParameterSource();
	...
	public List<Cart> findCartBySearchPage(String searchType, String searchvalue, PageBean pageBean) {
		...
		if (searchType.equals("adminusername")) {//如果搜索类型为按用户名称搜索
			sql = "select c.*,a.adminuser_username as adminuser_username,f.course_name as goods_name,f.course_images as goods_images,"
				+ "f.course_price as goods_price from interest_cart c,interest_adminuser a,interest_course f "
				+ "where c.adminuser_id=a.adminuser_id and c.goods_id=f.course_id and a.adminuser_username like '%"+searchvalue+"%' limit " + pageBean.getStart()
					+ "," + pageBean.getPageSize() + "";
		}
		List<Cart> CartList = namedParameterJdbcTemplate.getJdbcOperations().query(sql,new BeanPropertyRowMapper<Cart>(Cart.class));
		return CartList;
	...
	public int CartCountBySearch(String searchType, String searchvalue) {
		...
		if (searchType.equals("adminusername")) {
			sql = "select count(*) total from interest_cart c,interest_adminuser a "
	...
	public List<Cart> findCarts(Cart cart) {
		String sql = "select c.*,a.adminuser_username as adminuser_username,f.course_name as goods_name,f.course_images as goods_images,"
				+ "f.course_price as goods_price from interest_cart c,interest_adminuser a,interest_course f "
				+ "where c.adminuser_id=a.adminuser_id and c.goods_id=f.course_id ";
		List<Cart> CartList = namedParameterJdbcTemplate.getJdbcOperations().query(sql,new BeanPropertyRowMapper<Cart>(Cart.class));
		return CartList;
	}
	...
	public Cart getCartById(int cart_id) {
		String sql = "select c.*,a.adminuser_username as adminuser_username,f.course_name as goods_name,f.course_images as goods_images,"
				+ "f.course_price as goods_price from interest_cart c,interest_adminuser a,interest_course f "
				+ "where c.adminuser_id=a.adminuser_id and c.goods_id=f.course_id and c.cart_id=:cart_id";
		MapSqlParameterSource sps = new MapSqlParameterSource();
		sps.addValue("cart_id", cart_id);
	...
	public List<Cart> findCartsByAdminUserId(int adminuser_id) {
		String sql = "select c.*,a.adminuser_username as adminuser_username,f.course_name as goods_name,f.course_images as goods_images"
				+ ",f.course_price as goods_price from interest_cart c,interest_adminuser a,interest_course f "
				+ "where c.adminuser_id=a.adminuser_id and c.goods_id=f.course_id and c.adminuser_id=:adminuser_id";
		MapSqlParameterSource sps = new MapSqlParameterSource();
	...
	public int CartSumByAdminUser(int adminuser_id) {
		String sql = "select sum(cart_price) as sumprice from interest_cart where adminuser_id=:adminuser_id";
		int totals = 0;
		MapSqlParameterSource sps = new MapSqlParameterSource();
		sps.addValue("adminuser_id", adminuser_id);
		totals = (int) namedParameterJdbcTemplate.queryForLong(sql, sps);
	...
	public int deleteCartByAdminuserId(int adminuser_id) {
		String sql = "delete from interest_cart where adminuser_id=:adminuser_id";
		MapSqlParameterSource sps = new MapSqlParameterSource();
		sps.addValue("adminuser_id", adminuser_id);
		return namedParameterJdbcTemplate.update(sql, sps);
	}
	...
	public List<Cart> findCartsByAdminUserIdAndGoodsId(int adminuser_id, int goods_id) {
		String sql = "select * from interest_cart where adminuser_id=:adminuser_id and goods_id=:goods_id";
		MapSqlParameterSource sps = new MapSqlParameterSource();
		sps.addValue("adminuser_id", adminuser_id);
	...
	public int updateCartByAdminUserIdAndGoodsId(int adminuser_id, int goods_id) {
		String sql = "update interest_cart set cart_addnum=cart_addnum+1 where adminuser_id=:adminuser_id and goods_id=:goods_id";
		MapSqlParameterSource sps = new MapSqlParameterSource();
		sps.addValue("adminuser_id", adminuser_id);
		sps.addValue("goods_id", goods_id);
		return namedParameterJdbcTemplate.update(sql, sps);
	...
	public int updateCartByIdToUpdateNum(int cart_id, String cart_price, int cart_addnum) {
		String sql = "update interest_cart set cart_price=:cart_price,cart_addnum=:cart_addnum "
